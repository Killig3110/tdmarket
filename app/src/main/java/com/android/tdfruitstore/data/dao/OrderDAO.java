...
public class OrderDAO {
    ...
    // 🔹 Lấy danh sách đơn hàng theo userId (String)
    public void getOrdersByUserId(String userId, FirestoreCallback<List<Order>> callback) {
        db.collection(COLLECTION_NAME)
                ...
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    ...
                    for (QueryDocumentSnapshot doc : queryDocumentSnapshots) {
                        ...
                        order.setId(doc.getId()); // 🔥 Lưu ID từ Firestore vào object
                        orders.add(order);
                    }
    ...
    public void getAllOrders(FirestoreCallback<List<Order>> callback) {
        db.collection(COLLECTION_NAME)
                ...
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    ...
                    Log.d("Firestore", "✅ Fetched all orders");
                    callback.onSuccess(orders);
                })
                .addOnFailureListener(e -> {
                    Log.e("Firestore", "❌ Lỗi khi lấy toàn bộ đơn hàng: " + e.getMessage());
                    callback.onFailure(e);
                });
    }

    public void hasDeliveredOrder(String orderId, String productId, FirestoreCallback<Boolean> callback) {
        db.collection("order_details")
                .whereEqualTo("orderId", orderId)
                .whereEqualTo("productId", productId)
                .whereEqualTo("status", "DELIVERED") // 🔥 Kiểm tra trạng thái giao hàng
                .get()
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    boolean isDelivered = !queryDocumentSnapshots.isEmpty(); // ✅ Nếu có bản ghi với status = "DELIVERED"
                    Log.d("Firestore", "✅ Trạng thái giao hàng của order " + orderId + ", sản phẩm " + productId + ": " + (isDelivered ? "ĐÃ GIAO" : "CHƯA GIAO"));
                    callback.onSuccess(isDelivered);
                })
                .addOnFailureListener(e -> {
                    Log.e("Firestore", "❌ Lỗi khi kiểm tra trạng thái giao hàng: " + e.getMessage(), e);
                    callback.onFailure(e);
                });
    }
    ...
    public void insertOrderDetails(String orderId, List<OrderDetail> orderDetails, FirestoreCallback<Boolean> callback) {
        db.collection("order_details")
                .add(orderDetails)
                .addOnSuccessListener(documentReference -> {
                    Log.d("Firestore", "✅ Order details added with ID: " + documentReference.getId());
                    callback.onSuccess(true);
                })
                .addOnFailureListener(e -> {
                    Log.e("Firestore", "❌ Lỗi khi thêm order details: " + e.getMessage());
                    callback.onFailure(e);
                });
    }
